...
import { ActivityEditModal } from '../activity-edit-modal/activity-edit-modal';

@Component({
  selector: 'app-activity-crud',
  imports: [DataTable, SearchForm, ActivityEditModal],
...
export class ActivityCrud {
  ...
  showEditModal = false; // controla si se muestra el modal
  editingActivity: Activity | null = null; // actividad que se está editando
  ...
  /** Guarda actividad editada */
  saveEdit(newValues: any) {
    ...
    this.miServicio.update(updated).subscribe(() => {
